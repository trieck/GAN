import matplotlib.cm as cm
import matplotlib.pyplot as plt
import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import DataLoader
from torchvision import datasets, transforms
from torchvision.utils import make_grid

# ======== HYPERPARAMETERS ========
latent_dim = 100
batch_size = 512
epochs = 100
device = 'cuda' if torch.cuda.is_available() else 'cpu'

# ======== DATASET ========
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize([0.5], [0.5])  # scale to [-1, 1]
])

fixed_z = torch.randn(9, latent_dim, device=device)

dataset = datasets.MNIST(root='./data', train=True, transform=transform, download=True)
loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)


# ======== MODELS ========
class Generator(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.ConvTranspose2d(100, 512, 4, 1, 0),  # 1x1 → 4x4
            nn.BatchNorm2d(512),
            nn.ReLU(True),

            nn.ConvTranspose2d(512, 256, 4, 2, 1),  # 4x4 → 8x8
            nn.BatchNorm2d(256),
            nn.ReLU(True),

            nn.ConvTranspose2d(256, 128, 4, 2, 1),  # 8x8 → 16x16
            nn.BatchNorm2d(128),
            nn.ReLU(True),

            nn.ConvTranspose2d(128, 64, 4, 2, 1),  # 16x16 → 32x32
            nn.BatchNorm2d(64),
            nn.ReLU(True),

            nn.Conv2d(64, 1, kernel_size=5, stride=1, padding=0),  # 32x32 → 28x28
            nn.Tanh()
        )

    def forward(self, z):
        z = z.view(z.size(0), z.size(1), 1, 1)
        return self.net(z)


class Discriminator(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=4, stride=2, padding=1),  # 28 → 14
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1),  # 14 → 7
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.3),

            nn.Conv2d(128, 256, kernel_size=4, stride=2, padding=1),  # 7 → 4
            nn.BatchNorm2d(256),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.3),

            nn.Conv2d(256, 512, kernel_size=4, stride=2, padding=1),  # 4 → 2
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),

            nn.AdaptiveAvgPool2d(1),  # forces spatial output to 1×1
            nn.Flatten(),
            nn.Linear(512, 1),
        )

    def forward(self, x):
        return self.net(x)


# ======== INIT ========
G = Generator().to(device)
D = Discriminator().to(device)

loss_fn = nn.BCEWithLogitsLoss()
opt_D = optim.Adam(D.parameters(), lr=0.0001)  # slower D
opt_G = optim.Adam(G.parameters(), lr=0.0002)  # faster G

D_loss = torch.tensor(0.0, device=device)
G_loss = torch.tensor(0.0, device=device)

# ======== TRAINING LOOP ========
for epoch in range(epochs):
    for real_imgs, _ in loader:
        real_imgs = real_imgs.to(device)
        batch_size = real_imgs.size(0)

        # === Train Discriminator ===
        z = torch.randn(batch_size, latent_dim, device=device)
        fake_imgs = G(z).detach()
        real_labels = torch.full((batch_size, 1), 0.9, device=device)
        fake_labels = torch.zeros(batch_size, 1, device=device)

        noise_strength = max(0.1 * (1 - epoch / 50), 0.02)  # decay over time
        real_imgs += noise_strength * torch.randn_like(real_imgs)
        fake_imgs += noise_strength * torch.randn_like(fake_imgs)

        real_loss = loss_fn(D(real_imgs), real_labels)
        fake_loss = loss_fn(D(fake_imgs), fake_labels)
        D_loss = real_loss + fake_loss

        opt_D.zero_grad()
        D_loss.backward()
        torch.nn.utils.clip_grad_norm_(D.parameters(), max_norm=1.0)

        opt_D.step()

        # === Train Generator ===
        z = torch.randn(batch_size, latent_dim, device=device)
        gen_imgs = G(z)
        G_loss = loss_fn(D(gen_imgs), torch.ones(batch_size, 1, device=device))  # always target 1.0

        opt_G.zero_grad()
        G_loss.backward()
        opt_G.step()

    print(f"[Epoch {epoch + 1}/{epochs}] D_loss: {D_loss.item():.4f} G_loss: {G_loss.item():.4f}")

    # === Show sample ===
    with torch.no_grad():
        samples = G(fixed_z).cpu()  # [9, 1, 28, 28]

        # Apply magma colormap to each sample
        colored_samples = []
        for img in samples:
            img = img.squeeze(0)  # [28, 28]
            img = (img + 1) / 2  # [-1, 1] → [0, 1]
            img_np = img.numpy()

            # Apply magma colormap and convert back to tensor
            img_magma = cm.magma(img_np)[..., :3]  # ignore alpha channel
            img_tensor = torch.tensor(img_magma).permute(2, 0, 1)  # [H, W, C] → [C, H, W]
            colored_samples.append(img_tensor)

        colored_batch = torch.stack(colored_samples)  # [9, 3, 28, 28]
        grid = make_grid(colored_batch, nrow=3)

        # Show it
        plt.imshow(grid.permute(1, 2, 0))
        plt.axis('off')
        plt.title(f"Samples at epoch {epoch + 1}")
        plt.show()
